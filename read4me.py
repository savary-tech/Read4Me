...
Just run:  python3 tts_reader.py

Supported TTS backends (auto-detected in priority order):
  1. Kokoro   — local neural TTS, ~80 MB one-time download, no internet after
  2. edge-tts — Microsoft neural voices, free, requires internet
  3. pyttsx3  — offline system voices (espeak/festival)

Install (pick at least one):
  pip install kokoro soundfile sounddevice pypdf   # best quality, local
  pip install edge-tts pypdf                       # great quality, online
  pip install pyttsx3 pypdf                        # offline fallback

Optional (for edge-tts audio playback):
...
def arrow_menu(title, options, default=0):
    ...
    def _run(stdscr):
        ...
        while True:
            ...
            stdscr.addstr(2, 2, "\u2191/\u2193 navigate   Enter select   q cancel",
            ...
            if n > visible:
                ...
                try:
                    stdscr.addstr(h - 1, 2, hint, curses.color_pair(C_DIM))
                except curses.error:
                    pass
...
def clean_text(raw, flow_mode=False):
    ...
    # Remove lone page numbers (bare digits on their own line)
    text = re.sub(r'(?m)^\s*\d+\s*$', '', text)

    if flow_mode:
        # Collapse soft newlines (PDF line-wraps) into spaces.
        # A "real" paragraph break is 2+ newlines — preserve those.
        # Single newlines that follow a line ending in a word character
        # (i.e. not a sentence-ending punctuation + space) are soft wraps.
        # Strategy: protect double-newlines, collapse single ones, restore.
        text = re.sub(r'\n{2,}', '\x00PARA\x00', text)   # mark real breaks
        text = re.sub(r'\n', ' ', text)                   # collapse soft wraps
        text = re.sub(r' {2,}', ' ', text)                # tidy extra spaces
...
def step_file():
    """Step 1: ask for file path with tab completion."""
...
def step_pages(path):
    """Step 2 (PDF only): ask for page range."""
    total = pdf_page_count(path)
    if total <= 1:
        return None

    banner()
    print("  Step 2 of 5 \u2014 Pages")
    hr()
    print(f"  The PDF has {total} page(s).\n")
    options = [
        ("All pages",    f"Read all {total} pages"),
        ("Custom range", "Choose a start and end page"),
    ]
    ...
    while True:
        raw = ask("Page range (e.g.  3-10  or  7)")
        parts = raw.split("-")
        try:
            if len(parts) == 1:
                n = int(parts[0])
...
def step_backend(available):
    """Step 3 (skip if only one backend): pick TTS engine."""
    if len(available) == 1:
        return available[0]
...
def step_voice(backend):
    """Step 4: pick voice. Returns (voice_id, extra)."""
    banner()
    print("  Step 4 of 5 \u2014 Voice")
    hr()
    print()
...
def step_flow_mode():
    """Ask whether to use flow mode (collapse soft newlines)."""
    banner()
    ...
    if choice < 0: sys.exit(0)
    return choice == 0   # True = flow mode


def step_speed():
    """Step 5: pick speed."""
    banner()
    print("  Step 5 of 5 \u2014 Speed & Output")
    hr()
    print()
    speed_options = [
...
def step_output():
    """Ask play now or save to file. Returns save_path or None."""
    options = [
...
def show_summary(path, backend, voice_id, speed, page_range, save_path, words, est_min, flow_mode):
    ...
    print(f"  Flow    : {'on \u2014 soft newlines collapsed' if flow_mode else 'off \u2014 newlines preserved'}")
...
def main():
    ...
    if backend == "kokoro":
        kokoro_speak(text, voice_id, extra, speed, save_path)
    elif backend == "edge":
        edge_speak(text, voice_id, speed, save_path)
    else:
        pyttsx3_speak(text, voice_id, speed, save_path)
